...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Moonbase)`
    : `const run: (${functionSource.client}: Moonbase) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.collections.items.upsert',
    ...
    'client.inboxMessages.create',
    'client.inboxMessages.delete',
