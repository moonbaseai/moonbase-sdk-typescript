  ...
  inputSchema: {
    ...
    properties: {
      ...
      include: {
        type: 'array',
        description:
          'Specifies which related objects to include in the response. Valid options are `organizer` and `attendees`.',
        items: {
          type: 'string',
          enum: ['organizer', 'attendees', 'transcript'],
        },
      },
      jq_filter: {
        type: 'string',
...
export const handler = async (client: Moonbase, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.meetings.retrieve(id, body)));
  } catch (error) {
