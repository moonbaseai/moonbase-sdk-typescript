...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      values: {
        type: 'object',
        description: 'A hash where keys are the `ref` of a `Field` and values are the new data to be set.',
        additionalProperties: true,
      },
    ...
    required: ['collection_id', 'id', 'values'],
  },
  annotations: {},
};
...
export const handler = async (client: Moonbase, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.collections.items.update(id, body)));
};
